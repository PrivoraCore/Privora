...
*Setup instructions for a Gitian build of Privora Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Privora
...
- [Building Privora Core](#building-privora-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-privora-host/README.md).
...
Clone the git repositories for privora and Gitian.
...
in the Privora repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Privora Core
----------------

To build Privora Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the privora repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/privora/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/privora/privora
...
For example:
```bash
URL=https://github.com/snguyen-vn/privora.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit privora=${COMMIT} --url privora=${URL} ../privora/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit privora=${COMMIT} --url privora=${URL} ../privora/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit privora=${COMMIT} --url privora=${URL} ../privora/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the privora git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../privora/contrib/gitian-descriptors/*|sort|uniq )
...
VORAPATH=/some/root/path/privora
SIGPATH=/some/root/path/privora-detached-sigs

./bin/gbuild --url privora=${VORAPATH},signature=${SIGPATH} ../privora/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/privora-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/privora-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/privora-osx-build.assert
```
...
[privora-core/gitian.sigs](https://github.com/privora-core/gitian.sigs/) repository, or if that's not possible create a pull
