...
###   This script attempts to download the signature file SHA256SUMS.asc from privora.org
...
WORKINGDIR="/tmp/privora"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://privora.org/bin/"
VERSIONPREFIX="privora-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'privora-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Privora Core binary release signing key installed?"
   fi
