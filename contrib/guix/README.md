# Bootstrappable Privora Core Builds

This directory contains the files necessary to perform bootstrappable Privora
Core builds.
...
To build Privora Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Privora Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/privora-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/privora-core/guix.sigs).
