#!/bin/bash
# Copyright (c) 2013 The Privora Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Privora database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Privora datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Privora datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Privora 0.7 datadir."
    ;;
  3)
    echo "Detected Privora pre-0.8 datadir."
    ;;
  4)
    echo "Detected Privora 0.8 datadir."
    ;;
esac
