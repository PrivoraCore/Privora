...
The RPM spec file provided here is for Privora-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://privora.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Privora git repository](https://github.com/privora/privora) but are not part of
...
Currently only `Source10` is used, for the example `privora.conf` file.
...
in the privora git repository. At present only an SVG version of the Privora
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the privora
...
needed when 0.12.1 is released, a proper fix is already in the Privora git
master branch.
...
idea to build Privora Core against a static version of that library compiled
...
The `privora.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `privora-server` package. At this point, most RPM based
...
The `privora-server` package should have SELinux support. How to properly do
...
Privora-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/privora/). He has been
packaging Privora for Fedora at least since 2012.
...
4. I renamed the `privora` package that contains the Qt GUI to `privora-core` as
that appears to be how the general population refers to it, in contrast to
`privora-xt` or `privora-classic`. I wanted to make sure the general population
