...
`qa/pull-tester/test_framework/test_framework.py:PrivoraTestFramework.main`.
...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_PRIVORAD' not in vars():
    ENABLE_PRIVORAD=0
...
#Set env vars
if "PRIVORAD" not in os.environ:
    os.environ["PRIVORAD"] = BUILDDIR + '/src/privorad' + EXEEXT
if "PRIVORACLI" not in os.environ:
    os.environ["PRIVORACLI"] = BUILDDIR + '/src/privora-cli' + EXEEXT

if EXEEXT == ".exe" and "-win" not in opts:
    # https://github.com/privora/privora/commit/d52802551752140cf41f0d9a225a43e84404d3e9
    # https://github.com/privora/privora/pull/5677#issuecomment-136646964
    print("Win tests currently disabled by default.  Use -win option to enable")
    sys.exit(0)

if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_PRIVORAD == 1):
    print("No rpc tests to run. Wallet, utils, and privorad must all be enabled")
    sys.exit(0)
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie privorads, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `privora-cli help` (`rpc_interface.txt`).
