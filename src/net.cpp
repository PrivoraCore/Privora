...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Privora users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Privora " + FormatFullVersion();
...
void CConnman::ThreadDandelionShuffle() {
    ...
    while (!interruptNet) {
        if (GetTimeMicros() > nNextDandelionShuffle) {
            ...
            // results to not being able to close privora.
