...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total privora balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwallet && pwallet->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CPrivoraAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given Privora address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The Privora address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The Privora address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Privora address and we have full public key:
        CPrivoraAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are Privora addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) Privora address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CPrivoraAddress address(innerID);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The Privora address to use for the signature.\n"
...
bool getAddressFromIndex(AddressType const & type, const uint160 &hash, std::string &address)
{
    if (type == AddressType::payToScriptHash) {
        address = CPrivoraAddress(CScriptID(hash)).ToString();
    } else if (type == AddressType::payToPubKeyHash || type == AddressType::payToExchangeAddress) {
        address = CPrivoraAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CPrivoraAddress address(params[0].get_str());
        uint160 hashBytes;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CPrivoraAddress address(it->get_str());
            uint160 hashBytes;
            AddressType type = AddressType::unknown;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue getanonymityset(const JSONRPCRequest& request)
{
    ...
    if(!GetBoolArg("-mobile", false)){
        throw std::runtime_error(std::string("Please rerun Privora with -mobile "));
    }
...
UniValue gettotalsupply(const JSONRPCRequest& request)
{
    ...
    total -= 16810168037465;// burnt Coins sent to unrecoverable address https://explorer.privora.org/tx/0b53178c1b22bae4c04ef943ee6d6d30f2483327fe9beb54952951592e8ce368
    UniValue result(UniValue::VOBJ);
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "privora-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Znode features */
    { "privora",              "znsync",                 &mnsync,                 true,  {} },
    { "privora",              "evoznsync",              &mnsync,                 true,  {} },

    { "privora",              "verifyprivatetxown",      &verifyprivatetxown,      true,  {} },
