        ...
        if (tx.IsCoinBase()) {
            ...
            if (GetTransaction(txin.prevout.hash, prevTx, Params().GetConsensus(), hashBlock, true)) {
                ...
                if(ExtractDestination(txOut.scriptPubKey, dstAddr))
                    in.push_back(Pair("address", CPrivoraAddress(dstAddr).ToString()));
            }
        }
        if (tx.HasWitness()) {
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the Privora address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"aGoK6MF87K2SgT7cnJFhSWt7u2cAS5m18p\"   (string) Privora address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    ...
    if (type.isStr() && type.get_str() != "scripthash") {
        ...
        r.push_back(Pair("p2sh", CPrivoraAddress(CScriptID(script)).ToString()));
    }
