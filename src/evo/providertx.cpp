...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        collateralHash = ptx.collateralOutpoint.hash;
    } else {
        collateralHash = tx.GetHash();
    }

    if (Params().GetConsensus().blacklistedCollaterals.count(collateralHash)) {
        LogPrintf("Blacklisted collateral detected: %s (protx: %s) at height %d\n", collateralHash.GetHex(), tx.GetHash().GetHex(), pindexPrev ? pindexPrev->nHeight + 1 : -1);
        return true;
    }
