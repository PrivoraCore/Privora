...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    else
    {
        ...
        if (nNet > 0)
        {
            ...
            if (CPrivoraAddress(rec->address).IsValid())
            {
                CTxDestination address = CPrivoraAddress(rec->address).Get();
    ...
    if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty())
    {
        ...
        strHTML += "<b>" + tr("To") + ":</b> ";
        CTxDestination dest = CPrivoraAddress(strAddress).Get();
    ...
    if ((wtx.IsCoinBase() ||  wtx.tx->IsZerocoinSpend() || wtx.tx->IsSigmaSpend() || wtx.tx->IsZerocoinRemint()) && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += PrivoraUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (fAllFromMe)
        {
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.tx->vout)
            {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty())
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        ...
                        strHTML += GUIUtil::HtmlEscape(CPrivoraAddress(address).ToString());
                ...
                if(wtx.tx->IsSparkSpend() && wallet->validateSparkAddress(sparkOutput.address)) {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, -sparkOutput.amount) + "<br>";
                } else {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                }

                if(toSelf) {
                    if(wtx.tx->IsSparkSpend() && wallet->validateSparkAddress(sparkOutput.address)) {
                        strHTML += "<b>" + tr("Credit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, sparkOutput.amount) + "<br>";
                    } else {
                        strHTML += "<b>" + tr("Credit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
                    }
                }
            }
            ...
            if (fAllToMe)
            {
                if (wtx.tx->IsLelantusJoinSplit()) {
                    strHTML += "<b>" + tr("Total debit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, -wtx.tx->GetValueOut()) + "<br>";
                    strHTML += "<b>" + tr("Total credit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, wtx.tx->GetValueOut()) + "<br>";
                } else {
                    // Payment to self
                    CAmount nChange = wtx.GetChange();
                    CAmount nValue = nCredit - nChange;
                    strHTML += "<b>" + tr("Total debit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                    strHTML += "<b>" + tr("Total credit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
                }
            }
            ...
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.tx->vin)
                if (wallet->IsMine(txin, *wtx.tx))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, -wallet->GetDebit(txin, *wtx.tx, ISMINE_ALL)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.tx->vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + PrivoraUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
            ...
            {
                {
                    ...
                    if (ExtractDestination(vout.scriptPubKey, address))
                    {
                        if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].name.empty())
                            strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address].name) + " ";
                        strHTML += QString::fromStdString(CPrivoraAddress(address).ToString());
                    }
                    strHTML = strHTML + " " + tr("Amount") + "=" + PrivoraUnits::formatHtmlWithUnit(unit, vout.nValue);
