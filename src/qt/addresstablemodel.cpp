...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        CTxDestination curAddress = CPrivoraAddress(rec->address.toStdString()).Get();
        ...
        if(index.column() == Label)
        {
            ...
            if(rec->addressType == AddressTableModel::Spark){
                ...
                CTxDestination newAddress = CPrivoraAddress(value.toString().toStdString()).Get();
...
{
    // Update address book model from Privora core
    priv->updateEntry(address, label, isMine, purpose, status);
}

//[privora] AddressTableModel.updateEntry()
void AddressTableModel::updateEntry(const QString &pubCoin, const QString &isUsed, int status)
{
    // Update stealth address book model from Privora core
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, const QString &addressType)
{
    ...
    if(type == Send)
    {
        if(addressType == AddressTableModel::Spark) {
            ...
            {
                LOCK(wallet->cs_wallet);
                if(wallet->mapAddressBook.count(CPrivoraAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                }
    ...
    else if(type == Receive)
    {
        if(addressType == AddressTableModel::Spark) {
            ...
            strAddress = CPrivoraAddress(newKey.GetID()).ToString();
        }
    }
    else
    {
    ...
    // Add entry
    {
        ...
        if(addressType == AddressTableModel::Spark) {
            ...
            wallet->SetAddressBook(CPrivoraAddress(strAddress).Get(), strLabel,
                                (type == Send ? "send" : "receive"));
        } else if(addressType == AddressTableModel::RAP) {
...
QString AddressTableModel::labelForAddress(const QString &address) const
{
    {
        LOCK(wallet->cs_wallet);
        CPrivoraAddress address_parsed(address.toStdString());
