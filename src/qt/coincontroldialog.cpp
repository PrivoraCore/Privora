    ...
    anonymousMode(anonymousMode)
{
    ...
    // see https://github.com/privora/privora/issues/5716
...
{
    GUIUtil::setClipboard(PrivoraUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog, bool anonymousMode)
{
    ...
    int nDisplayUnit = PrivoraUnits::VORA;
    ...
    l2->setText(PrivoraUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(PrivoraUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(PrivoraUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(PrivoraUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CPrivoraAddress(outputAddress).ToString());

                // if listMode or change => show Privora address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, PrivoraUnits::format(nDisplayUnit, amount));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, PrivoraUnits::format(nDisplayUnit, nSum));
