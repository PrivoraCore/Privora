// Copyright (c) 2010 Satoshi Nakamoto// Copyright (c) 2016-2019 The Privora Core developers
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Privora address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"privoraaddress\"    (string) The new Privora address\n"
            "\nExamples:\n"
    ...
    return CPrivoraAddress(keyID).ToString();
}
...
UniValue getnewexchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() == 0) {
        ...
        CPrivoraAddress existingKey(request.params[0].get_str());
...
CPrivoraAddress GetAccountAddress(CWallet * const pwallet, std::string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CPrivoraAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Privora address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"privoraaddress\"   (string) The account Privora address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Privora address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "setaccount \"privoraaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"privoraaddress\"  (string, required) The Privora address to be associated with an account.\n"
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "getaccount \"privoraaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"privoraaddress\"  (string, required) The Privora address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"privoraaddress\"  (string) a Privora address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    for (const std::pair<CPrivoraAddress, CAddressBookData>& item : pwallet->mapAddressBook) {
        const CPrivoraAddress& address = item.first;
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            "sendtoaddress \"privoraaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"privoraaddress\"  (string, required) The Privora address to send to.\n"
            ...
            "                             The recipient will receive less privoras than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"privoraaddress\",     (string) The Privora address\n"
    ...
    for (std::set<CTxDestination> grouping : pwallet->GetAddressGroupings()) {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CPrivoraAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwallet->mapAddressBook.find(CPrivoraAddress(address).Get()) != pwallet->mapAddressBook.end()) {
                    addressInfo.push_back(pwallet->mapAddressBook.find(CPrivoraAddress(address).Get())->second.name);
                }
            }
...
UniValue listaddressbalances(const JSONRPCRequest& request)
{
    ...
    for (auto& balance : balances)
        if (balance.second >= nMinAmount)
            jsonBalances.push_back(Pair(CPrivoraAddress(balance.first).ToString(), ValueFromAmount(balance.second)));
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signmessage \"privoraaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"privoraaddress\"  (string, required) The Privora address to use for the private key.\n"
    ...
    CPrivoraAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "getreceivedbyaddress \"privoraaddress\" ( minconf addlocked )\n"
            "\nReturns the total amount received by the given privoraaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"privoraaddress\"  (string, required) The Privora address for transactions.\n"
    ...
    // Privora address
    CPrivoraAddress address = CPrivoraAddress(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Privora address");
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a privora address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The Privora address to send funds to.\n"
    ...
    std::string strAccount = AccountFromValue(request.params[0]);
    CPrivoraAddress address(request.params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Privora address");
    CAmount nAmount = AmountFromValue(request.params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The Privora address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less privoras than you enter in their corresponding amount field.\n"
    ...
    std::set<CPrivoraAddress> setAddress;
    std::vector<CRecipient> vecSend;
    ...
    {
        CPrivoraAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Privora address: ")+name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Privora address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keys\"         (string, required) A json array of Privora addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) Privora address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"address\"         (string) A Privora address associated with the keys.\n"
    ...
    return CPrivoraAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    return CPrivoraAddress(w.result).ToString();
}
...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool fByAccounts, bool fAddLocked)
{
    ...
    // Tally
    std::map<CPrivoraAddress, tallyitem> mapTally;
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        const CWalletTx& wtx = pairWtx.second;
    ...
    for (const std::pair<CPrivoraAddress, CAddressBookData>& item : pwallet->mapAddressBook) {
        ...
        std::map<CPrivoraAddress, tallyitem>::iterator it = mapTally.find(address);
...
static void MaybePushAddress(UniValue & entry, const CTxDestination &dest, CPrivoraAddress &addr)
{
    if (addr.Set(dest))
                ...
                if (wtx.IsCoinBase())
                {
                    ...
                    if (!fSkipMnpayoutCheck) {
                        ...
                        for(CTxOut const & out : voutMasternodePaymentsRet) {
                            ...
                            CPrivoraAddress payeeAddr(payeeDest);
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"privoraaddress\",    (string) The Privora address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"privoraaddress\",    (string) The Privora address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"address\" : \"privoraaddress\",   (string) The Privora address involved in the transaction\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() != 2)) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending privoras\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->IsCrypted() && (request.fHelp || request.params.size() != 1)) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending privora\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"privoraaddress\" \"test message\"") +
    ...
    return "wallet encrypted; Privora server stopping, restart to run with encrypted wallet.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending privoras.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("settxfee", "0.00000001 PRIVORA")
            + HelpExampleRpc("settxfee", "0.00000001 PRIVORA")
        );
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of Privora addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) Privora address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the Privora address\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CPrivoraAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Privora address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CPrivoraAddress(address).ToString()));
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The Privora address to receive the change\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CPrivoraAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid Privora address");
...
UniValue regeneratemintpool(const JSONRPCRequest& request) {
    ...
    if(reindexRequired)
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Mintpool issue corrected. Please shutdown privora and restart with -reindex flag.");

    return true;
}

//[privora]: sigma/lelantus section
...
UniValue spendmany(const JSONRPCRequest& request) {
    ...
    for (const auto& strAddr : keys) {
        CPrivoraAddress address(strAddr);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Privora address: " + strAddr);
...
extern UniValue dumpprivkey_privora(const JSONRPCRequest& request); // in rpcdump.cpp
extern UniValue importprivkey(const JSONRPCRequest& request);
extern UniValue importaddress(const JSONRPCRequest& request);
extern UniValue importpubkey(const JSONRPCRequest& request);
extern UniValue dumpwallet_privora(const JSONRPCRequest& request);
...
UniValue sendtorapaddress(const JSONRPCRequest& request)
{
    ...
    CPrivoraAddress address = pwallet->GetTheirNextAddress(theirPcode);
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "dumpprivkey",              &dumpprivkey_privora,        true,   {"address"}  },
    { "wallet",             "dumpsparkviewkey",         &dumpsparkviewkey,        true,   {}  },
    { "wallet",             "dumpwallet",               &dumpwallet_privora,         true,   {"filename"} },
